...
describe("endpoint validation", () => {
  ...
  it("injects trusted-edge identity only into its pinned localhost origin", async () => {
    ...
    const opts = clientOptions("http://127.0.0.1:8093", { env, fetchImpl });

    await opts.fetch("http://127.0.0.1:8093/v1/kv?key=a", {
      headers: { "x-request-id": "req-1" },
    });
    assert.equal(captured.input, "http://127.0.0.1:8093/v1/kv?key=a");
    ...
    await assert.rejects(
      opts.fetch("http://127.0.0.1:8094/v1/kv?key=a"),
  ...
  it("refuses the trusted-edge escape hatch outside an explicit localhost harness", () => {
    ...
    assert.throws(
      () => clientOptions("http://127.0.0.1:8093", { env: baseEnv }),
      /require HTTPS/,
    );
  });
  ...
  it("refuses ambiguous credentials and missing local identity", () => {
    ...
    assert.throws(
      () => clientOptions("http://127.0.0.1:8093", {
