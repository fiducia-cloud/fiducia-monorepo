...
test(
  "local harness enforces HTTP and security boundaries",
  { timeout: 10_000 },
  async () => {
    ...
    try {
      ...
      const rejected = await fetch(`${harness.origin}/healthz`, {
        method: "POST",
      });
    ...
    try {
      ...
      if (page) {
        await Promise.allSettled([
          page.screenshot({
            path: await artifactPath("playwright", "harness-contract.png"),
            fullPage: true,
          }),
          page
            .content()
            .then((content) =>
              writeArtifact("playwright", "page.html", content),
            ),
          writeArtifact(
            "playwright",
            "browser-errors.json",
