...
test(
  "Puppeteer satisfies the browser harness contract",
  ...
  async () => {
    ...
    assert(
      executablePath,
      "PUPPETEER_EXECUTABLE_PATH must identify the CI Chrome binary",
    );
    ...
    try {
      ...
      const response = await page.goto(harness.origin, {
      ...
      if (page) {
        await Promise.allSettled([
          page.screenshot({
            path: await artifactPath("puppeteer", "harness-contract.png"),
            fullPage: true,
          }),
          page
            .content()
            .then((content) =>
              writeArtifact("puppeteer", "page.html", content),
            ),
          writeArtifact(
            "puppeteer",
            "browser-errors.json",
